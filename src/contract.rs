...
pub fn instantiate(
    ...
    let config = Config {
        starbreeder_token: deps.api.addr_canonicalize(&msg.starbreeder_token)?,
...
pub fn receive_cw20(
    ...
    if config.starbreeder_token != deps.api.addr_canonicalize(info.sender.as_str())? {
...
/// returns error if the project id is invalid
fn validate_project_id(prj_id: &Option<String>) -> StdResult<()> {
    if let Some(link) = link {
        if link.len() < MIN_LINK_LENGTH {
            Err(StdError::generic_err("Link too short"))
        } else if link.len() > MAX_LINK_LENGTH {
...
pub fn end_poll(deps: DepsMut, env: Env, poll_id: u64) -> StdResult<Response> {
    ...
    let (quorum, staked_weight) = if state.total_share.u128() == 0 {
        ...
        let staked_weight = load_token_balance(
            &deps.querier,
            deps.api.addr_humanize(&config.starbreeder_token)?.to_string(),
            &state.contract_addr,
        )?
    ...
    if tallied_weight == 0 || quorum < config.quorum {
        ...
        if !a_poll.deposit_amount.is_zero() {
            messages.push(CosmosMsg::Wasm(WasmMsg::Execute {
                contract_addr: deps.api.addr_humanize(&config.starbreeder_token)?.to_string(),
                funds: vec![],
...
fn query_config(deps: Deps) -> StdResult<ConfigResponse> {
    ...
    Ok(ConfigResponse {
        ...
        starbreeder_token: deps.api.addr_humanize(&config.starbreeder_token)?.to_string(),
    ...
    let poll_responses: StdResult<Vec<PollResponse>> = polls
        ...
        .map(|poll| {
            Ok(PollResponse {
                ...
                project_id: poll.project_id.clone(),
                deposit_amount: poll.deposit_amount,
